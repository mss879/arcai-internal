...
function TileBody({ metric }: { metric: ArtifactField }): React.ReactElement {
      ...
      {artifact.metrics.map((metric, i) => {
        ...
        return (
          <button
            ...
            className={cn(
              shell,
              "group relative text-left transition hover:-translate-y-0.5 hover:border-primary-200 hover:shadow-[var(--shadow-lift)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-300",
            )}
          >
            <TileBody metric={metric} />
            <ChevronRight
              aria-hidden
              className="absolute right-3 top-4 h-4 w-4 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-primary-500"
            />
          </button>
        );
      })}
