...
// The engine's pure decisions live in automation-core.ts so they can be unit
// tested without this module's `server-only` import. They are re-exported here
// because project-events.ts, delivery.ts and the automation UI have always
// imported them from the engine.
export { passesConditions, renderTokens };
